...
export default function LibraryGrid({ }: LibraryGridProps) {
      ...
      <div className="relative rounded-3xl overflow-hidden bg-slate-900 mt-6">
        ...
        <div className="relative z-10 px-6 py-10 md:px-12 md:py-20 max-w-2xl">
          <h2 className="text-3xl md:text-5xl font-black text-white leading-tight mb-4 animate-in slide-in-from-left duration-700">
            {t('knowledgePower')} <br />
            <span className="text-brand-primary">{t('ownForever')}</span> on Web3.
          </h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 animate-in slide-in-from-left delay-200 duration-700">
            <button className="bg-brand-primary hover:bg-brand-secondary text-white px-8 py-4 rounded-2xl font-black transition-all shadow-xl shadow-brand-primary/20 active:scale-95">
              {t('exploreNow')}
            </button>
      ...
      <div className="relative">
        {loading && books.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center gap-4">
            ...
            <button onClick={() => handleSearch()} className="text-brand-primary hover:underline text-sm font-bold">{t('retry')}</button>
          </div>
        ) : (
                  ...
                  <div className="relative aspect-3/4 bg-slate-50 overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      <div className="bg-white text-slate-800 font-black py-3 px-6 rounded-2xl text-xs shadow-xl flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0 transition-all duration-500">
                        <Sparkles className="w-4 h-4 text-brand-primary" /> {t('buyNow')}
                      </div>
                    </div>
                  </div>

                  <div className="p-5 flex flex-col grow">
                    <h3 className="text-slate-900 font-black text-sm line-clamp-2 mb-1 group-hover:text-brand-primary transition-colors leading-tight">{book.title}</h3>
                    ...
                    <div className="mt-auto flex items-center justify-between">
                      <div className="flex flex-col">
                        ...
                        <span className="text-lg font-black text-slate-900">
                          {formatPrice(book.priceMist)} <span className="text-xs text-brand-primary">SUI</span>
                        </span>
                      </div>
                      <div className="w-10 h-10 rounded-2xl bg-brand-primary/10 text-brand-primary flex items-center justify-center group-hover:bg-brand-primary group-hover:text-white transition-all shadow-sm group-hover:shadow-lg group-hover:shadow-brand-primary/30">
            ...
            {totalPages > 1 && (
              <div className="mt-16 flex items-center justify-center gap-3">
                <button
                  ...
                  className={`p-4 rounded-2xl border border-slate-100 transition-all ${currentPage === 1 ? 'opacity-30 cursor-not-allowed' : 'bg-white hover:shadow-xl text-brand-primary'}`}
                >
                ...
                <div className="flex items-center gap-2">
                  {[...Array(totalPages)].map((_, i) => (
                    <button
                      ...
                      className={`w-12 h-12 rounded-2xl font-black transition-all ${currentPage === i + 1 ? 'bg-brand-primary text-white shadow-xl shadow-brand-primary/30' : 'bg-white border border-slate-100 text-slate-400 hover:text-brand-primary'}`}
                    >
                      {i + 1}
                    </button>
                  ))}
                ...
                <button
                  ...
                  onClick={() => setCurrentPage(p => p + 1)}
                  className={`p-4 rounded-2xl border border-slate-100 transition-all ${currentPage === totalPages ? 'opacity-30 cursor-not-allowed' : 'bg-white hover:shadow-xl text-brand-primary'}`}
                >
